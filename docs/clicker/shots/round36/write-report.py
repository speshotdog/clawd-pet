...
all_runs=[read(f'final-{i}-verification.json') for i in range(1,6)]
rows=[entry['round36'] for run in all_runs for entry in run]
a=rows[0];m=a['baselineMetrics'];before=read('baseline-dev.json')['stats']
def span(key,places=4):
 v=[r[key] for r in rows];return f'{min(v):.{places}f}～{max(v):.{places}f}'
lines=['''# 第三十六輪：魔花少女的字色、卡框與懸浮粒子
...
| 量測 | GPU 改前 | 改後 | 變化／門檻 |
''']
for title,key,rule in [('卡框平均灰階','ringMean','+12%～+30%'),('卡框灰階標準差','ringStd','至少保留 95%'),('整卡平均灰階','cardMean','增幅 ≤5%'),('圖窗中央角色區平均灰階','centerMean','增幅 ≤2%')]:
 lines.append(f"| {title} | {before[key]:.6f} | {a['stats'][key]:.6f} | {a['gainsPercent'][key]:+.6f}%；{rule} |\n")
lines.append('''
...
|---|---|---|
''')
for title,key,unit,rule in [('圖窗間隔 2 秒的差異像素','motionPercent','%','≥0.15%，灰階差 >6'),('文字框差異像素','plateMotionPercent','%','≤0.02%'),('粒子開／關峰值增亮','particlePeak',' 灰階','≥8'),('粒子相對鄰近背景的峰值','particleNeighborPeak',' 灰階','≥8'),('粒子導致整卡平均增亮','particleGainPercent','%','≤2%'),('靜置 3 秒的 rAF 次數','rafs','','0'),('reduced-motion 間隔截圖差異','reducedPercent','%','≤0.02%'),('傾斜時粒子容器超緣','tiltedParticleOverflow','px','≤1.5px')]:
 lines.append(f'| {title} | {span(key)}{unit} | {rule} |\n')
lines.append('''| 正常時粒子容器超緣 | −9.004578px | ≤1.5px |
...
curve_summary=[]
for i in range(1,6):
 code=int((E/f'final-{i}.exit').read_text(encoding='utf-8-sig').strip());assert code==0
 counts=[]
 for label in ['dev','moved-copy']:
  curve=read(f'final-{i}-{label}-curve.json');gains=[1,curve['gain'],.72]
  n=[min(sum(r['face']==face and r['ms']>1 and .05<r['opacity'][k]/g<.95 for r in curve['frames']) for k,g in enumerate(gains)) for face in ['alt','idle']]
  error=max(abs(r['opacity'][k]/g+r['opacity'][k+3]/g-1) for r in curve['frames'] for k,g in enumerate(gains))
  curve_summary.append({'run':i,'entry':label,'frames':len(curve['frames']),'entryMinimum':n[0],'returnMinimum':n[1],'maxSumError':error})
  counts.append(f'{n[0]}／{n[1]}')
 lines.append(f'| {i} | {code} | {counts[0]} | {counts[1]} | [final-{i}.log](shots/round36/final-{i}.log) |\n')
(E/'crossfade-summary.json').write_text(json.dumps(curve_summary,indent=1),encoding='utf-8')
lines.append(f"\n十個入口每條曲線錄得 {min(x['frames'] for x in curve_summary)}～{max(x['frames'] for x in curve_summary)} 格，最大互補和誤差 {max(x['maxSumError'] for x in curve_summary):.8f}，雙空／雙滿均為 0。各入口錯誤與外部 HTTP 請求皆為空陣列；原有兩份火箭珍幾何／Z／字級比例、剪影對齊、實際點擊切換與指標像素回應驗收亦通過。見 [crossfade-summary.json](shots/round36/crossfade-summary.json)。\n")
lines.append('''
## 紅燈、真實退出碼與交付
...
''')
(E.parents[1]/'REPORT-holo-round36.md').write_text(''.join(lines),encoding='utf-8')
print('Wrote Chinese round36 report; five exits verified as zero.')
