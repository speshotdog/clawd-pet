...
window.CeremonyTable = Object.freeze({
  ...
  layout(n,{width=innerWidth,height=innerHeight,index=0}={}){
    ...
    const cw=phone?Math.min(n===1?304:288,width-64):n===1?Math.max(190,375*Math.min(width/1440,height/900)):smallDesktop?Math.max(170,Math.min(n>5?198:208,(width*.88-32)/5,(height-148)/(n>5?2.8:1.4))):Math.max(190,Math.min(n>5?198:208,(width*.88-32)/5,(height-140)/(n>5?3.0:1.7)));
