...
## 2026-09-13 之後這一輪：觀星動態卡（素材做好，等使用者看過再決定要不要接進卡池）

待辦裡的「觀星日後升級動態」。走**不摳圖**那條路（使用者 2026-09-13 定案：摳不出來就不要摳）：
整幅畫的 5:7 視窗直接動起來，不做逐幀主體／alpha，所以沒有去背品質的風險。

- `build_star_motion.py`：來源 `source-5.0/觀星系列 拆四張卡.mp4`（1080×1920、60fps、29.5s），
  以靜卡取的第 120 格（2.000 秒）為中心取 3 秒、24fps，**視窗完全沿用 `prepare_5_0_flat.py` 的
  CARDS**（同一個臉中心、同一個高度），來回播（ping-pong）避免接點跳。
- 輸出動畫 WebP，可直接放進 `<img>`：

| id | 名稱 | 視窗 | 格數 | 大小 |
|---|---|---|---:|---:|
| `wangyuanmie` | 咩有看錯星（神話）| (79, 932, 521, 1552) | 142 | 1.20 MB |
| `xingyejiao` | 今晚不睡膠（神話）| (434, 830, 1076, 1730) | 142 | 0.93 MB |
| `shanqiulong` | 星願龍總欸（傳說）| (640, 737, 940, 1157) | 142 | 0.32 MB |

- 比對頁：`shots/5.0/star-motion-preview.html`（自包含）。中心格與現有靜卡逐像素同構圖，已比對過。
- **還沒接進卡池**，而且不建議在使用者看過之前接：`card_assets.py` 是「一張卡一份編碼」的權威檔，
  它會 `Image.open(...).convert('RGBA')` 重存，動畫會被壓成一格。要接就得加一條「動畫檔原樣帶過」的分支，
  並重跑 `check_card_identity.py` 與整條像素驗收——那是凍結檔案，值得單獨一輪。
