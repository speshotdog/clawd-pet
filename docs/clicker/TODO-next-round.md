...
**回家接手先讀 [ISSUES-2026-09-09-night.md](ISSUES-2026-09-09-night.md)**，六項開著的問題與選項都在那裡：
四張新神話卡改做 depth 立體卡（補洞路線待裁決）、背景線條加光暈、1024 留白規格矛盾、
咩噗熊亮邊判準、史詩光效偏弱、`4.0\怪物\` 未處理。
工法與今晚的改動見 [HANDOFF-2026-09-09-night.md](HANDOFF-2026-09-09-night.md)。

---

# 第三十三輪：召喚介面與特效

A–J 的實作、逐項數據、實際退出碼及交付邊界見 [REPORT-holo-round33.md](REPORT-holo-round33.md)。正式／測試各有 dev 與 standalone；測試版由既有 builder 的 `--test` 旗標產生。

- 待裁決：1024px 寬度無法同時容納五張 ≥190px 卡、四個 ≥8px 間隙及左右各約 99.6px 留白。目前保留五欄與可讀性，1024 的五／十連左右留白未達標；checker 保留失敗，不能當成已批准例外。
- I-3 仍有界外擴散限制：474／516px 是指定取樣幀的外徑，後段淡出的環仍會經過視口邊界。本輪保留 A 的直徑與節奏，未宣稱整段環都位於畫面內；須連同小桌面留白一起裁決。
- Round32 的六個素材失敗仍在：咩噗熊亮邊比例、五張 flat 的 crop-only。卡面凍結，本輪不修改原畫或門檻。
- 分層契約已依本輪裁決更新：其他卡 → 背光／射線 → 演出卡 → 衝擊波。背光只排除演出卡，應蓋過鄰卡；環需跨越所有卡，並保持文字對比。
⚠ 2026-09-10 更正：結果卡寬不是 245px。實碼是桌面五連 208px、十連 198px、
小桌面下探 170px（`ceremony-layout.js:11`），舊報告的 245／190 數字已過期。

- 抽卡後先退場 220ms、推近 380ms，再無限等待點擊卡包。固定驗收時間以開封時刻為第二個起點，不能把等待改成自動逾時。
- 正式版 RATE 與初始 30 券保留；測試版頁面有明顯標記、∞ 券。不可拿測試版當正式像素／時序基準。
- 未提交、未推送。Git 共用 metadata 位於可寫 worktree 外；下一次有適當權限時依報告的邊界提交，不要收進使用者原有的 BRIEF 修改。
...
- 歷史 round31 契約：Canvas 永遠在卡後、排除所有卡面 rect。本條已由 round33 的四層契約取代，不能重新套用到本輪。
