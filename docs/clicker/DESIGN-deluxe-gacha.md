...
以下是使用者對第 9 節問題的回答與追加設定。**這一批只做規劃記錄，細節之後再做**；
目前優先處理卡片本身的問題。未被回答的題目仍然待定。

### 10.1 已定案

| # | 問題 | 使用者定案 |
|---|---|---|
| 5 | 精美版是第二次抽樣還是必定精美？ | **精裝池每張都是精美版。** 沒有「精裝池裡抽到普通版」這回事 |
| 7 / 8 | 保底 | **新招募方式不會有保底。** 不沿用第 30／40 抽那套，也不共用 `sinceLegendary` |
| 14 | 卡冊 | **兩個版本可切換：一般卡冊 與 精裝卡冊** |
| 2 / 3 | 貨幣 | **末世抽獎券要用末世金幣買**；**舊版金幣也可以直接換抽獎券**（比值待定）。原則是**不讓舊資源變成沒用** |

### 10.2 追加設定：末世地圖（新的解鎖與分流）

- **打完現有關卡的人才會進到「末世地圖」**，那是新區域。
- **末世地圖只能用精裝版的卡戰鬥。**
- **末世與舊版是兩個分開的區域，資源也不同**（兩套金幣）。
- 精裝抽卡、精裝卡冊、末世地圖**都要打完最後一關才開啟**。

### 10.3 這批定案帶出的新問題（尚未回答，不要自行決定）

- 「打完最後一關」對應現有存檔的哪個條件？目前終點站是第七站滅世都市
  （`economy.canBoss`／`finishBoss`／`save.validate` 三處有終點規則），
  是以打贏滅世珍獸為準，還是另設條件？
- 精裝卡是**同一批角色的精美版**，還是**末世地圖專屬的新角色**？
  （第 4 題仍未回答，但因為「只能用精裝卡戰鬥」，這題現在會影響戰鬥平衡）
- 兩個區域各自的進度、場景、王、印記是各自獨立還是共用？
- 舊金幣換末世抽獎券的**比值**（使用者說之後思考）。
- 既然沒有保底，神話率是否仍是 0.5%？沒有保底的池子在長尾玩家身上的體感要不要模擬過？
- 精美版重複怎麼處理（第 11 題）仍未回答。

### 10.4 追加定案（2026-09-08，同日稍晚）

- **精裝卡會有舊角色的卡，但卡面是新的精裝版。**（第 4 題部分回答）
- **以後會出精裝版專屬的卡**（新角色只在精裝池）。
- **精裝版無法在舊地圖使用，兩邊戰力是區分的。**
  → 精裝版不是「舊卡的強化版」，是**另一套獨立戰力**。
  → 第 12 題（精美版是否只改外觀）因此有了答案的一半：**不只改外觀，它是另一個區域的戰鬥資源**。
- **舊的卡面維持舊版樣式，不更新了，當成區別。**
  ⚠ 這一條**推翻**了 `BRIEF-holo-cards-research.md` 第十節的「舊卡也要一起升級」。
  新的箔面／景深／寶石／字體是**精裝版的識別**，不是全體升級。
  ⚠ 但「舊版樣式」具體指哪一版**尚待使用者確認**，見 10.5。

### 10.5 末世地圖：進入條件與第一個玩法點

- **進入條件：打完最後一關 BOSS。**（回答 10.3 的第一個問題）
  現行終點是第七站滅世都市的滅世珍獸，終點規則散在
  `economy.canBoss`／`economy.finishBoss`／`save.validate` 三處，接線時三處都要看。

- **第一張圖：點擊 50 次會送一隻新的精裝卡「普發月月」**（素材之後補）。
  這是新手引導性質的保底禮，讓玩家一進末世就有一張能戰的精裝卡。
- 待定：這張卡的稀有度、是否可重複取得、是否計入卡冊完成度、
  50 次是「累計」還是「單場」、送出時是否走裝幀典藏的演出。

### 10.6 因這批定案而需要重新確認的事

- ~~進入條件對應存檔哪個條件~~ → 已答：**打完最後一關 BOSS**。
- **「舊版樣式」是哪一版？** 兩種讀法會導致完全不同的工作量：
  (a) 遊戲現在線上的卡面，完全不動；
  (b) 展示頁第四輪那個「卡框＋稀有度色調底」的版型（使用者當時說「照圖二我認為就很好了」）。
  在使用者確認前，**不要動 `src/` 的既有卡面**。
- 兩套戰力分開之後，舊地圖的既有平衡（訓練、羈絆、技能、印記）是否原封不動？
- 精裝卡在卡冊裡是否也顯示戰力，還是只有末世地圖看得到？
