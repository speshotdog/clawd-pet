...
## 十八、2026-09-08 深夜：第三十三輪（已出貨，main `0cb8b1a`／gh-pages `fd26878`）

**接手第一步**：`git fetch && git pull --ff-only`，`npm test`（170 例），
以及 kickoff 列的**十一支** Playwright 套件（新增 `clicker-round33.py`）。

### 一、略過改成「快轉」，只有沒抽過的卡會停下來

使用者定案：**只有新卡停，轉彩與升星里程碑都不停。**

按略過鍵之後照抽卡順序掃過去：重複卡一張一張翻正（間隔 50ms），
沒抽過的卡先煞車 180ms、再把它的翻牌演出完整播完，播完繼續掃。
判定用 `draw` 裡凍結的 `item.dup`（`gacha-pool.js:170`，同一包抽到兩張同角色只有第一張算新），
**不需要任何新的存檔欄位**。

四個刻意的設計決定，改的時候不要順手拿掉：

1. **順序照扇形由左到右**。先全部翻正再回頭補播新卡的話，新卡會在一個已經全開的扇形裡
   孤零零翻一次，讀起來像 bug 不像強調。
2. **重複卡不做「瞬間全部翻正」**。快轉要是一個看得見的動作，否則玩家覺得這顆鍵時靈時不靈。
3. **停下來時 `#recruit-hint` 打「新夥伴・○○○」**。沒有這行字，第一反應是快轉壞了。
4. **逃生口**：快轉中按鈕字樣變「全部略過」，再按一次就是原本的硬略過。

⚠ **`restore()` 仍走硬略過（`runtime.skip()`），絕對不要改成快轉。**
那是「還原已經看過的結果」，重播新卡演出會讓玩家以為自己又抽了一次。round33 有守這條。

⚠ **快轉是在「重建的乾淨 runtime」上跑的**（跟 `restore()` 同一條路，所以五種演出模式行為一致，
不必去拆 rip／stage 各自的 skip 語意）。**一定要先把五張卡建出來再進迴圈**——
`reveal(key)` 第一行是 `cards.get(key)`，查不到就直接 return。
我第一版漏了這步：四張新卡 600ms 就「跑完」、提示字全部閃過去，但一張演出都沒播。
**只看提示字會以為它動了，要看翻開張數有沒有一張一張增加。**

⚠ `fastForward()` 裡設旗標與改按鈕字樣的兩行**必須排在 `makeRuntime()` 後面**：
`makeRuntime()` 內部會跑 `cleanup()`，而 `cleanup()` 會把它們重置回去。

### 二、拆包面上不該有任何其他熱區

起點是使用者說「技能的設計可能還是離開點擊畫面比較好」。出題給 Astra 討論
（`DISCUSS-skill-slots.md` ／ `-astra.md`），**Astra 糾正了我兩件事，查證都對**：
重疊比例橫直式都是 56.25%（我算成 59% 是用取整後的 DOMRect）；冷卻中的技能是 `disabled`，
按不下去，所以「誤觸浪費冷卻」不成立，真正的代價是誤放一個**已就緒**的技能。

但 Astra 是純讀原始碼推的（它自己講明沒跑實機）。我把 `#tap` 打成 2px 網格逐點問
`elementFromPoint`，結果比兩邊原本想的都糟——**珍母身上只有 85.6% 真的在拆包**：
...
| `#daily-bag`（今日限定包） | 4.3% |
| 技能槽容器／`#slots`（靜默吃掉，什麼都不做） | 3.1% |

所以這不是「技能槽的位置問題」，是**「拆包面上不該有任何其他熱區」**。做了：

- **`#slots` 搬出 `#stage`**，改成 `#stage-fit` 的兄弟。
  橫式用遊戲座標 `(44,296)` 擺回原位（舞台原點 16,72 ＋ 舞台內 28,224），
  1280×860 下四個槽仍在 95／195／295／395 @400，**一分不差**。
  直式自成一列排在舞台下面（64 高，矮螢幕 56）。
- **今日限定包**從 `x=300..380` 搬到 `x=30..110`（原本跟拆包鍵疊 3680 舞台 px²）。
- `#slots` 與 `.skill-slot` 改 `pointer-events:none`，只有 `.skill-use` 可點。

結果：**直式 100% 都是拆包**，橫式 93.3%（只剩技能鍵本身）。

⚠ **搬 `#slots` 會連帶動到兩個地方**：
`clicker-extras.js` 原本用 `$('slots').before(regenTag)` 插入回升標，
但 `#regen-tag` 的 CSS 是 `#stage > #regen-tag`——`#slots` 離開舞台之後不能再當插入點，
已改成 `stageEl.append(regenTag)`。另外 `#slots` 的舊定位是 `bottom:72px`（相對舞台），
搬家之後要改成 `top:296px`（相對 `#game`）。

⚠ **直式的代價**：固定高度多了 64+7=71，伸縮留白從 151.5 縮到 80.5，
**舞台與拆包鍵一起上移 71px（拆包中心 533 → 462）**。
如果實玩覺得拆包鍵太高，把 `#team` 改 `order:3`、`#stage-fit` 改 `order:4` 就變成
「夥伴 → 舞台 → 技能」（Astra 推薦的 D 案），拆包中心回到 561，代價是夥伴列上移 307。
**這是一行的事，留給實玩決定。**

⚠ **橫式還沒解的 6.7%** 就是技能鍵本身。橫式的舞台裡沒有一塊 304×64 的乾淨空地
（實測：往下移會撞到進度條與子包，左邊直排會蓋掉旗子與連鎖標），
Astra 的方案是把技能挪進夥伴區右側、夥伴每頁從十人改五人——那要動 `setPartners()` 的分頁、
翻頁與入隊落點，是獨立一輪的事。桌機是滑鼠，誤觸機率遠低於拇指，先擱著。

### 三、直式的落點換算（已上線的 bug，Astra 順手發現的）

**三連包在直式，點第一包或第二包都會開到第三包。** 落點原本是
`(clientX - gameBox.left) / (gameBox.width / 960)`——那只有橫式成立，
直式 `#game` 是整個畫面（390 寬）、舞台是另外縮的 608 框，整整放大 1.5 倍。
實測：點第一包（螢幕 x=242）算出舞台 x=579，離 504 最近 → 開第三包。

改成 `pointOf()`，從 `#stage` 自己的盒子換算再加回舞台原點 `(16,72)`。
橫式代回去就是原本的式子（`stage.left = game.left + 16×zoom`、`k = zoom`），**數字一分不差**；
直式也對。`#tap`／`#boss-view`／子包與禮包熱區三處共用同一份。

### 四、矮螢幕

直式的固定高度**原本就已經超過 667**（第三十二輪就把底部分頁擠出畫面外了，
而 `#game-content` 是 `overflow:hidden`，被擠出去的分頁是**按不到**、不是捲得到），
加一條技能列之後更糟。新增 `@media (max-aspect-ratio:3/4) and (max-height:700px)` 的壓縮版面
（錢包 48、商店三列 48、技能列 56、夥伴 88、分頁 48、列距 5），
390×667 與 320×640 的分頁都回到畫面內，round33 有守。

### 這一輪學到的坑

- **「重疊」不等於「會誤觸」，要逐點量。** 矩形交集算出來的比例（56.25%）跟實際會發生什麼
  是兩件事：按鈕是圓的、容器會攔截、冷卻中是 disabled。
  `document.elementFromPoint` 打網格才是真的——而且它讓我看到第三個熱區（`#daily-bag`），
  那是我跟 Astra 都沒想到的。
- **派 Astra 做純分析很好用，但它不跑實機。** 它會給算式、會糾正你的前提（這次兩條都對），
  但「畫面上實際會發生什麼」一定要自己開瀏覽器量。它的結論當假說，不當結果。
- **「跑完了」不等於「做了事」。** 快轉第一版 600ms 跑完、提示字全部出現過，
  看起來像成功——實際上 `reveal()` 每次都在第一行 return。
  驗收要盯**狀態有沒有一步一步改變**（翻開張數 0→1→2→3→4→5），不要只看終點與副作用。
