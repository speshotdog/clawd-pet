...
| `tools/sim/clicker-boss.js` | 王包難度模擬（後院；三～六場景還沒加） |

## 五、數值定案（與簡報不同處）

- 王包係數全部 `boss.mul 1.35`（簡報的 6 是尺度誤解：王包需求是「30 秒產出對一包需求」）。
- 場景解鎖是**每場景**拆滿 60／70／80／90 包再打王（需求每場景從第 1 包重算，1.12^k 撐不到 200 包）。
- 夥伴訓練價 `50×1.22^L`（1.5 會讓 100 級要 2e19 幣）。
