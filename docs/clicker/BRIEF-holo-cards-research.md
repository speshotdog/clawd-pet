...
## 十、使用者定案二（2026-09-08，方向與產品目標）

1. **走滿版場景卡**（第九節那個問題已定案，不用再評估兩種要哪種）。
2. **舊的 40 張卡也要一起升級**，不是只有新卡好看。舊卡是單層去背圖，
   要想辦法讓它們在滿版景深系統下也變漂亮（生背景？程式產底？卡框補景深？——你提方案）。
3. **使用者要先看到東西才決定。**
   → 所以實作第一輪的產出**不是直接改遊戲**，而是一個**可以互動玩的展示頁**：
   同一批卡、放上新效果，滑鼠轉一轉就看得到箔面與景深，讓使用者當場拍板。
   展示頁要包含：舊卡升級後的樣子、新場景卡的樣子、五個稀有度並排比較。
4. **產品終局**（設計要往這個方向收）：
   - 舊卡變漂亮（既有卡池，全體升級）
   - **新增一個新卡池**，抽得到「非常精美版本」的卡——這是進階轉蛋的賣點，
     精美版要跟普通版有明顯的檔次差（不只是多一層光）。
   - 所以「卡片外觀」必須是**資料驅動、可以按卡池分檔次**的，不是寫死在稀有度上。
     同一個角色可能同時存在普通版與精美版。
