...
def shot(p):
 p.evaluate('__syncAnimations();__ceremony.syncFx()');return Image.open(BytesIO(p.screenshot())).convert('RGB')
def frozen_bg(p):
...
EXPECTED={
 'common':{'foil-sweep':(0,220)},
 'rare':{'foil-sweep':(0,300)},
 'epic':{'foil-sweep':(0,420)},
 'legendary':{'fx-substrate-wave':(40,900),'stage-impact':(80,160),'foil-sweep':(0,520)},
 'mythic':{'fx-spectrum-wave':(60,1100),'fx-counter-wave':(180,1200),'mythic-ripple':(0,1040),'stage-impact':(60,200),'foil-sweep':(0,560),'foil-afterglow':(480,440)}}

def neutral(browser,url,label):
 captures=[]
 for rarity in ['common','legendary','mythic']:
  p,errors=open_page(browser,url);frozen_bg(p);start(p,[BY['common'],BY[rarity],BY['common'],BY['common'],BY['common']]);advance(p,2490);captures.append(shot(p));captures[-1].save(OUT/f'{label}-next-neutral-{rarity}.png');assert not p.evaluate('__ceremony.events.some(e=>e.type==="face-visible"&&e.index===1)');assert not errors; p.close()
...
def core(browser,url,label):
 ...
 for w,h in [(1440,900),(1024,640),(390,844)]:
  for n in [1,5,10]:
   ...
   expected=min(304 if n==1 else 288,w-64) if w<600 else 420*min(w/1440,h/900) if n==1 else min((w-80)/5.55,(h-200)/(3.15 if n>5 else 1.7))
   assert all(abs(d['cw']-expected)<=1 for d in data),(w,h,n,data)
   layout=p.evaluate(f'__ceremony.layout({n})')
   if w>600 and n>1:
    for i,d in enumerate(layout):assert abs(d['x']-(i%5-2)*expected*1.14)<.01 and d['rz']==0
   if n==10 and w<600:
    p.click('#next');assert state(p)['selectedIndex']==1
   if w>600:assert p.locator('.slot.page-away').count()==0
 ...
 for rarity,expected in EXPECTED.items():
  ...
  advance(p,1619);assert not p.locator('.rarity-fx').count();assert not p.evaluate('__ceremony.events.some(e=>e.type==="face-visible")')
  ...
  for elapsed in [100,240,400,650,850]:
   ...
   if elapsed==400:
    ...
    p95=lambda v:sorted(v)[int(len(v)*.95)];ratio=p95(bg)/p95(card);assert p95(bg)>0,ratio # Round30 supersedes the dim-background ceiling with original-relative floors.
  advance(p,3000);ev=p.evaluate('__ceremony.events');F=next(e['time'] for e in ev if e['type']=='face-visible');end=next(e['time'] for e in ev if e['type']=='slot-complete')
  ...
  particles=next(e['particles'] for e in ev if e.get('effect')=='original-particles' and e['type']=='fx-start')
  assert particles>={'common':8,'rare':26,'epic':62,'legendary':68,'mythic':100}[rarity]
 ...
 for phase,ms in [('tension',300),('tear',550),('deal',900),('before-face',2069),('after-face',2071),('return',2480),('second-row',6000)]:
...
def main():
 ...
 with sync_playwright() as pw,TemporaryDirectory(prefix='portable29-',dir=OUT) as tmp:
  ...
  for label,file in [('dev',HERE/'deluxe-gacha-b.html'),('portable',copy)]:
   (live if '--live' in sys.argv else neutral if '--neutral-only' in sys.argv else core)(b,file.as_uri(),label)
  b.close()
