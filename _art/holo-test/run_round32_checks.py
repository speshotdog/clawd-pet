...
OUT=ROOT/'docs/clicker/shots/round32';OUT.mkdir(parents=True,exist_ok=True)
env=dict(os.environ,PYTHONIOENCODING='utf-8')
rows=[]
for arg in sys.argv[1:]:
    cmd=[sys.executable,str(ROOT/'_art/holo-test'/arg.split()[0]),*arg.split()[1:]]
    name=arg.replace(' ','_').replace('/','_')+'-'+str(time.time_ns())
    before=time.time()
    with (OUT/(name+'.log')).open('w',encoding='utf-8') as log:
        result=subprocess.run(cmd,cwd=ROOT,stdout=log,stderr=subprocess.STDOUT,env=env)
    row={'command':'python _art/holo-test/'+arg,'exit_code':result.returncode,'seconds':round(time.time()-before,2),'log':name+'.log'}
