...
本輪把文字框恢復為有重量的懸浮銘牌，但只更新 `depth` 與 `flat` 展示卡：

- `depth` 的前後順序是「圖繪背景 → 文字框 → 拆出的角色 → 名字／稀有度」。背景、文字框、角色各自使用自己的 Z 位移與視差；角色可以壓過文字框上緣，名字永遠在最上層。
- `flat`（滅世珍獸）沒有拆出的角色層，因此只有「背景圖 → 文字框 → 名字／稀有度」；展示頁不把它描述成角色浮出來。
- `framed` 維持舊版卡框樣式。

寶石直接沿用 `src/gacha-card.css` 的 `.face-gem` 形狀、圓角與基本漸層，只加入很輕的稀有度配色、邊光和角度反光。展示頁的「寶石微加工對照」區提供原版／加工後並排證據；加工後仍以對比度 35 作為驗證門檻。寶石疊在文字框左側，名字與稀有度仍以卡片中心對齊，不會被寶石推移，也不與寶石相交。

Standalone 已由 `build_round5_standalone.py` 重新建立：圖片內嵌、可直接以 `file://` 開啟、無外部請求，並控制在 15 MiB 內。`r8-` 截圖與 `verification-round8.json` 是本輪的實測證據；畫面是否達到最終視覺品質仍由使用者驗收。
