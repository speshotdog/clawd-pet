// v3 進度曲線模擬（DESIGN-balance-v3）：真人節奏（點擊 4/s、技能一好就放、貪婪買回本最快的升級、有錢就五連），
// 量：每站大王第一次打贏的時間（牆在哪）、小王卡了多久、輪迴時間與每輪印記、祝福幾輪封頂、三種打法過王比例、派遣收入。
// 用法：node tools/sim/clicker-v3.js [clicksPerSec] [sessionMin] [days]
// A/B：GATE_MUL=2 BOSS_MUL=4 node tools/sim/clicker-v3.js …（覆寫 B.V3 的常數）
...
function fight() {
  ...
  while(E.canBoss(s,now)&&tries<3) {
    ...
    if(tries===1) {   // 三種打法的 30 秒容量（技能冷卻全清、需求無限大）
      ...
      if(caps[2]<need*(.95+regenRate*30)) { s=E.abandonBoss(s,now); skipUntil=now+SESSION; stuckSince??=now; return; }
    }
    ...
    if(won) { firstWin[scene]??=now; stuckSince=null; return; } now+=31000; s=E.settle(s,now).state;
  }
}
...
while(now<DAYS*86400000) {
  ...
  s=E.settle(s,now).state; shop();
  try{ const r=E.recall(s,now); s=r.state; dispatchLog.recalls+=r.rewards.filter(x=>!x.capped).length; }catch{}
  for(const id of Object.keys(s.collection).filter(id=>!E.rosterOf(s).includes(id)&&!E.dispatched(s,id)).slice(0,3)) { try{ s=E.dispatch(s,id,now); }catch{} }
  if(now<86400000) daily.push(`${fmt(now)} 收工 ${s.settings.scene}#${s.package.index} P=${E.rates(s).P.toExponential(2)} D=${E.rates(s).D.toExponential(1)} 訓練Lv${s.trainingLevel} 攻擊力Lv${s.clickLevel} 夥伴${Object.keys(s.collection).length} Lv中位${[...Object.values(s.partnerLevels||{})].sort((a,b)=>a-b)[Object.keys(s.partnerLevels||{}).length>>1]||0}`);
...
for(const g of prestigeLog) console.log(' 換桌布 第',g.n,'次',fmt(g.at),'本輪 +'+g.gained,'累積印記',g.claimed,'祝福 Lv'+g.bless,'×'+g.blessMul.toFixed(2),'當家',g.champions.join('/'));
