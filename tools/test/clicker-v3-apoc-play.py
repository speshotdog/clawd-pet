...
with sync_playwright() as p:
    ...
    boss_mul = pg.evaluate("()=>{const m=ApocEconomy.RULES.BOSS_MULS; ApocEconomy.RULES.BOSS_MULS=m.map(()=>1000); Clicker.state.apoc.progress=3; return m;}"); pg.wait_for_timeout(1300)
    ...
    check((lost['stage'] is None or lost['stage'].get('farm')) and lost['failed'] == lost['progress'] == 3, f"時間到 → 王關失敗、記下這站輸過 {lost['failed']}")
    need3 = pg.evaluate("()=>ApocEconomy.need(3)")
    check(not lost['carry'], '輸了不保留血量（第五輪使用者：不要保留血量的機制）')
    pg.wait_for_timeout(1500)
    farm = pg.evaluate("()=>Clicker.state.apoc.stage")
    check(bool(farm) and farm.get('farm') and farm['index'] == 2 and not farm['boss'], f"輸了自動回前一站刷怪、不是再打王（第六輪：照 Sakura Clicker）：{farm and farm['index']}")
    check('刷怪中' in pg.locator('#package-label').inner_text(), '血條寫著刷怪中：' + pg.locator('#package-label').inner_text())
    # inner_text 讀得到不代表看得到：1.0 的標籤欄固定 72px、overflow hidden，以前「第 4 站・王關」就只露出「第 4 站・」
    clip = pg.evaluate("()=>{const e=document.getElementById('package-label'); return [e.scrollWidth, e.clientWidth];}")
    check(clip[0] <= clip[1] + 1, f'血條標籤整段看得到、沒有被切（scrollWidth {clip[0]}／clientWidth {clip[1]}）')
