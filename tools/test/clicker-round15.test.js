...
test('round15: 46 characters and 34 PNG assets', () => {
  assert.equal(Pool.CHARACTER_IDS.length,46); assert.equal(Object.keys(B.characters).length,46);   // 第二十二輪 +13
  assert.equal(Pool.RARITY_ORDER[0],'mythic'); assert.equal(Pool.RARITY.mythic.label,'神話');
  const png=Pool.CATALOG.filter(e=>e.kind==='char' && e.src); assert.equal(png.length,34);
