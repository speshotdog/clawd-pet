...
test('印記：floor(sqrt(生涯/1e8)) − 已領；倍率 1+.05×已領', () => {
  ...
  assert.ok(Math.abs(E.markMul(r.state) - (1 + .5 * Math.sqrt(2))) < 1e-9);   // 第二十三輪：線性 +5%／枚 → 1 + .5×√印記
  assert.throws(() => P.prestige(r.state, 0), /沒有可領/);
  S.validate(r.state, Pool);
});
test('倍率進 rates：P 與攻擊力部分都乘 M，不重複乘（4 枚印記＝1+.5×√4＝×2）', () => {
  const s = own(seed({ marksClaimed: 4, marks: 0, lifetimeCoins: 1e9, coins: 0 }), 'yueyue2', 1);
  const base = own(seed(), 'yueyue2', 1);
  assert.ok(Math.abs(E.rates(s).P / E.rates(base).P - 2) < 1e-9); assert.ok(Math.abs(E.rates(s).D / E.rates(base).D - 2) < 1e-9);
});
