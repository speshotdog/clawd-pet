# -*- coding: utf-8 -*-
"""2.0 推薦組合：卡冊有鍵、開得起來、模板照稀有度挑隊伍裡戰力最高的卡、套用會寫進技能槽、缺階會標缺。
...
def main():
    with sync_playwright() as p:
        ...
        pg.evaluate("""()=>{const A=ApocEconomy,s=Clicker.state; const a=A.normalize(s.apoc); const by=r=>ApocPool.filter(c=>(c.rarity==='common'?'rare':c.rarity)===r).map(c=>c.id);
          const ids=[...by('mythic').slice(0,2),...by('legendary').slice(0,3),...by('epic').slice(0,5),...by('rare').slice(0,5)];
        ...
        first = names[0]
        check('缺' not in first[2] and all('(缺)' not in s for s in first[1]), f'王關爆發：四階都在隊伍裡，全部到齊：{first[2]}')
        check(names[5][2].startswith('缺'), f'全神話：隊伍只有 2 張神話，標缺：{names[5][2]}')
        ...
        # 套用王關爆發 → 技能槽 = 史詩／神話／傳說／精良、各自是該階戰力最高
        pg.locator('#recommend-page .recommend-card').nth(0).locator('.recommend-apply').click(); pg.wait_for_timeout(800)
        got = pg.evaluate("""()=>{const A=ApocEconomy,a=A.normalize(Clicker.state.apoc); const rar=id=>{const c=ApocPool.find(c=>c.id===id);return c.rarity==='common'?'rare':c.rarity};
        ...
        check(got['rar'] == ['epic', 'mythic', 'legendary', 'rare'], f'套用後四格稀有度照模板：{got["rar"]}')
        check(all(got['best']), f'每格是該階戰力最高（或次高，同階同戰力）：{got["best"]}')
        check(not pg.evaluate("()=>document.getElementById('recommend-page').hidden"), '套用後頁面留著（重畫）')
        # 隊伍沒有精良、卡冊有 → 順手入隊
        pg.evaluate("""()=>{const A=ApocEconomy,s=Clicker.state; const a=A.normalize(s.apoc); const rare=a.roster.filter(id=>{const c=ApocPool.find(c=>c.id===id);return c.rarity==='rare'||c.rarity==='common'});
        ...
        check(bool(joined), '隊伍裡沒精良卡：從卡冊挑一張入隊並裝到第四格')
