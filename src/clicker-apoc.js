...
(function (root) {
  const STEPS = [
    ...
    { tab: 'map', title: '這是末世地圖', text: '線性一條路，打不死就不能進下一關。小怪放著也會打；王有護盾，護盾在場時放置只剩三成五，要靠點擊把盾點破——破防那幾秒全部傷害 ×2。' },
    ...
    { tab: 'team', title: '把卡拖到獨立技能格', text: '桌機從隊伍網格直接拖到「獨立技能 1～4」；手機點格子挑。技能看卡的稀有度：神話＝接下來 10 下 ×10、傳說＝15 下 ×3、史詩＝全隊 ×1.5 二十秒、精良＝其他技能冷卻 −8 秒。戰鬥時在地圖右邊按。' },
  ...
  function create({ $, store, commit, changed, notice }) {
    ...
    function apply(fn, silent = false) {
      ...
      pushState(); for (const ev of events) { if (ev.type === 'win') { post({ type: 'apoc-win', index: ev.index, reward: ev.reward }); notice(`末世：通過第 ${ev.index + 1} 站，+${ev.reward} 末世金幣`); } else if (ev.type === 'fail') { post({ type: 'apoc-fail', index: ev.index }); notice(`王關失敗，${Math.round(A.RULES.BOSS_COOLDOWN / 1000)} 秒後再挑戰`); } }
      return events;
    }
    ...
    addEventListener('message', e => {
      ...
      else if (d.type === 'apoc-skill') apply((a, now) => A.useSkill(a, d.slot | 0, now));
    });
