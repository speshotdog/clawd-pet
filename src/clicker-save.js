(function (root) {
  ...
  function validate(s, pool) {
    ...
    check(object(s.apoc) && typeof s.apoc.unlocked === 'boolean' && integer(s.apoc.tutorial) && (s.apoc.coins === undefined || (number(s.apoc.coins) && s.apoc.coins >= 0 && integer(s.apoc.tickets) && s.apoc.tickets >= 0 && integer(s.apoc.progress) && s.apoc.progress >= 0 && s.apoc.progress <= (s.apoc.endless || s.apoc.cleared ? (globalThis.ApocEconomy?.RULES.ENDLESS_MAX ?? 100) : 20))), '末世');   // 第十輪 C：無盡模式第 21 站起（以前卡 20，commit 被擋、整個畫面停住）；通關過的也放寬——關掉無盡後進度仍 >20（Codex 10C 必修）
