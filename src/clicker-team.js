...
(function (root) {
  function create({ $, store, commit, changed, action, notice, sound, card, E, B, Pool, format, drag }) {
    ...
    const rank = id => 3 - E.origin(id);   // 0 神話 … 3 精良
    let page = 0, selected = null, pickerMode = 'add', pickerSlot = 0, pending = null, previewTimer = 0;
    const team = () => E.rosterOf(store.state);
    const el = (tag, cls, text) => { const n = document.createElement(tag); if (cls) n.className = cls; if (text !== undefined) n.textContent = text; return n; };
    function portrait(id) { const p = el('span', 'buddy-portrait'); p.append(card.art.create(Pool.byId[id])); return p; }
    function proxy(id, index, handler) {
      const b = el('button', 'team-proxy'); b.type = 'button'; b.dataset.id = id;
      b.setAttribute('aria-label', `${index === null ? '候選' : String(index + 1).padStart(2, '0')} ${Pool.byId[id].name} ${RAR[Pool.byId[id].rarity]}`);
      ...
      img.append(el('span', 'proxy-symbol', SYMBOLS[Pool.byId[id].rarity]));
      b.append(img, el('span', 'proxy-name', Pool.byId[id].name.replace('（原版）', '')));
      if (E.champion(store.state, id)) b.append(el('i', 'champ-flag', '本輪當家'));
      const slot = store.state.skillSlots.indexOf(id); if (slot >= 0) b.append(el('i', 'slot-stamp', `槽${slot + 1}`));
      b.onclick = handler; return b;
    }
    // ---- 累加上限列（同 team20 renderCapacity）
    function renderCapacity(ids = team()) {
      const n = E.rosterCounts(ids), rows = $('t20-caps'); rows.replaceChildren();
    ...
    function renderSkills() {
      const s = store.state, box = $('t20-skills'); box.replaceChildren();
      for (let i = 0; i < s.skillSlots.length; i++) {
        const id = s.skillSlots[i], b = el('button', 'skill-slot'); b.type = 'button'; b.dataset.slot = i;
        b.append(el('small', '', `技能槽 ${i + 1}`));
        if (i >= E.slotCount(s)) { b.disabled = true; b.append(el('span', 'slot-name', '未解鎖')); }
        else if (id) { b.append(portrait(id), el('span', 'slot-name', Pool.byId[id].name.replace('（原版）', ''))); }
    ...
    function detail() {
      const s = store.state, ids = team(), id = selected && s.collection[selected] ? selected : ids[0];
      ...
      $('t20-detail-number').textContent = serial; $('t20-detail-name').textContent = Pool.byId[id].name; $('t20-detail-rarity').textContent = `${RAR[Pool.byId[id].rarity]}・每秒 ${format(E.individual(s, id))}${E.champion(s, id) ? '・本輪當家 ×1.5' : ''}`;
      $('t20-detail-op').textContent = `${serial} · ${index >= 0 ? '已編入' : '尚未編入'} · ${Pool.byId[id].name}`;
      const host = $('t20-card'); host.replaceChildren(); const face = card.create({ ...Pool.byId[id], rarity: E.rarity(s, id) }, { tag: false }); face.classList.add('flipped', 'album-card', 'detail-card'); host.append(face);
    ...
    function preview(id) {
      ...
      previewTimer = setTimeout(() => { const affected = LIMITS.map((_, i) => rank(id) <= i); document.querySelectorAll('#t20-caps .capacity-row').forEach((r, i) => r.classList.toggle('affected', affected[i])); $('t20-picker-status').textContent = `${Pool.byId[id].name} · 影響：${LABELS.filter((_, i) => affected[i]).join('、')}`; }, 120);
    }
    function add(id, replace = null) {
      clearOperation(); const s = store.state;
      if (!s.collection[id]) return false;
      if (E.dispatched(s, id)) { $('t20-picker-status').textContent = '派遣中，回來才能編入'; return false; }
      let next = team().filter(x => x !== replace);
      if (next.includes(id)) { $('t20-picker-status').textContent = '此成員已在隊伍中'; return false; }
      next.push(id);
      const bad = E.rosterViolations(next);
      if (next.length > 20 || bad.length) {
        const n = E.rosterCounts(team()), after = E.rosterCounts(next);
        const msg = next.length > 20 ? '隊伍已滿 20 張，先移出一張' : bad.map(i => `${LABELS[i]}合計已達 ${n[i]}，加入後將為 ${after[i]}（上限 ${LIMITS[i]}，用出身算）`).join('；');
      ...
      let ok = false;
      action(() => { if (commit(E.setRoster(s, next, Date.now()))) { ok = true; changed(); sound('upgrade'); } });
      if (ok) { selected = id; renderRoster(); detail(); notice(replace ? `${Pool.byId[replace].name} → ${Pool.byId[id].name}` : `${Pool.byId[id].name} 編入隊伍`); }
      return ok;
    }
    function remove() {
      const s = store.state, id = selected; if (!id || !team().includes(id)) return;
      action(() => { if (commit(E.setRoster(s, team().filter(x => x !== id), Date.now()))) { changed(); selected = team()[0] || null; renderRoster(); detail(); $('team-editor').classList.remove('detail-open'); notice(`${Pool.byId[id].name} 離開隊伍${s.skillSlots.includes(id) ? '，技能槽一併清空' : ''}`); } });
    }
    function assignSkill(i, id) {
      let ok = false;
      action(() => { try { const next = E.equip(store.state, i, id, Date.now()); if (commit(next)) { ok = true; changed(); sound('upgrade'); notice(`技能槽 ${i + 1} · ${Pool.byId[id].name}，等待 30 秒`); renderRoster(); } } catch (err) { notice(err.message); $('t20-picker-status').textContent = err.message; } });
      return ok;
    }
    ...
    function openPicker(mode, slot = 0) {
      ...
      const pool = mode === 'skill' ? ids : Object.keys(s.collection).filter(id => s.collection[id] > 0 && B.characters[id]).sort((a, b) => E.origin(b) - E.origin(a) || E.individual(s, b) - E.individual(s, a));
      const grid = $('t20-picker-grid'); grid.replaceChildren();
      for (const id of pool) {
        const b = proxy(id, null, () => { if (mode === 'skill') { pending = id; $('t20-picker-status').textContent = `技能槽 ${slot + 1} · ${Pool.byId[id].name}`; } else preview(id); $('t20-picker-confirm').disabled = false; document.querySelectorAll('#t20-picker-grid .team-proxy').forEach(x => x.setAttribute('aria-selected', String(x === b))); });
        if (mode !== 'skill' && ids.includes(id)) b.disabled = true;
        if (mode !== 'skill' && E.dispatched(s, id)) { b.disabled = true; b.title = '派遣中'; }
        grid.append(b);
      }
    ...
    $('team-auto').onclick = () => action(() => { const s = store.state, auto = E.autoRoster(s, s.skillSlots.filter(Boolean)); if (commit(E.setRoster(s, auto, Date.now()))) { changed(); selected = auto[0] || null; renderRoster(); detail(); notice('已照每秒收益自動編隊'); } });
