  ...
  function create({ $, store, commit, changed, notice, format, card, sound, openRoster, openTeam, openMarks, cutin }) {
    ...
    function renderMech(st) {
      ...
      if (!parts.hidden) {
        ...
        // 滿版改用「舞台的比例」直接釘在珍獸身上：頭＝臉（眼睛嘴巴）、身＝揮手的肚子、尾＝右側藍刺的末端。
        // 朋友 2026-09-16 第二次回報：舊值 body [.76,.46] 壓在羊角上、頭在臉右邊的空白處。
        // 數字是從桌機截圖量的舞台比例（不是原圖座標換算）：臉＝眼睛與嘴中間、羊角在 (.72,.47) 要避開。
        const full = enemy.classList.contains('full-board');
        const pos = full ? { head: [.53, .45], body: [.78, .70], tail: [.90, .60] } : { head: [-.28, -.14], body: [.02, .12], tail: [.4, -.02] };
